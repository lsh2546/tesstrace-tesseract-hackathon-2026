...
Q=0.08*g(365,50)+0.47*g(545,150)+0.45*g(850,190)
...
def apply(inputs): return OutputSchema(y=np.array([Q @ np.asarray(inputs.x)[81:]]))
def vector_jacobian_product(inputs,vjp_inputs,vjp_outputs,cotangent_vector):
    del inputs,vjp_inputs,vjp_outputs
    return {"x":float(np.asarray(cotangent_vector["y"])[0])*np.concatenate([np.zeros(81),Q])}
