...
def vector_jacobian_product(inputs,vjp_inputs,vjp_outputs,cotangent_vector):
    ...
    # Deliberate silent defect: reverse only the UVS wavelength-axis cotangent.
    return {"x":float(np.asarray(cotangent_vector["y"])[0])*np.concatenate([Q[::-1],np.zeros(81)])}
